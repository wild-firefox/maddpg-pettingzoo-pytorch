...
###
def child_process(conn,args):
    env = ToyEnv(render_mode=False)
    while True:
        maddpg,step = conn.recv()
        ...
        conn.send((step,info,agent_reward))
...
if __name__ == '__main__':
    ...
    ###
    process_num = 4
    pipe_dict = {i: Pipe() for i in range(process_num)}
    child_process_list = [Process(target=child_process, args=(pipe_dict[i][1],args)) for i in range(process_num)]
    ...
    while episode < args.episode_num+1:


        ###
        [pipe_dict[i][0].send((maddpg,step)) for i in range(process_num)]
        for i in range(process_num):
            episode +=1
            step,info,agent_reward = pipe_dict[i][0].recv()
            # episode finishes
