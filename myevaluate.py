...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode-length', type=int, default=60, help='steps per episode')
    ...
    env, dim_info = get_env(args.env_name,render_mode=args.render_mode)
    ...
    for episode in range(args.episode_num):
        ...
        #while env.agents:  # interact with the env for an episode
            action_nor = maddpg.select_action(obs)  # 注 混合博弈需要噪声 例:石头剪刀布
...
'''
    ...
    ax.set_xlabel('片段数')
    ax.set_ylabel('奖励值')
    title = f'reward function of evaluate result of maddpg solve {args.env_name} ({args.model_name} vs {args.policy_number})'
    title_zn = f'奖励函数曲线_{args.model_name}'
    ...
    ax.set_xlabel('片段数')
    ax.set_ylabel('胜负率')
    title = f'win_lose_rate of maddpg solve {args.env_name} ({args.model_name} vs {args.policy_number})'
    title_zn = f'胜负率曲线_{args.model_name}'
    ax.set_title(title_zn)
    title += f'{total_files}'
    plt.savefig(os.path.join(test_dir, title))
    ...
    ax.set_xlabel('片段数')
    ax.set_ylabel('大胜率')
    title = f'win1_rate of maddpg solve {args.env_name} ({args.model_name} vs {args.policy_number})'
    title_zn = f'大胜率曲线_{args.model_name}'
    ax.set_title(title_zn)
    title += f'{total_files}'
    plt.savefig(os.path.join(test_dir, title))
'''