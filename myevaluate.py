...
if __name__ == '__main__':
    ...
    parser.add_argument('--episode-num', type=int, default=200, help='total episode num during evaluation')
    ...
    total_files = len([file for file in os.listdir(test_dir)]) // 2 + 1
    ...
    ax.set_xlabel('片段数')
    ax.set_ylabel('奖励值')
    title = f'reward function of evaluate result of maddpg solve {args.env_name} '
    #title = '奖励函数曲线'
    ax.set_title(title)
    title += f'{total_files}'
